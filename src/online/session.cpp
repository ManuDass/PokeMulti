...
void campState(Bytes& b,const game::CampState& c){
    ...
    for(const auto& p:c.party){word(b,p.species);word(b,uint16_t(p.x));word(b,uint16_t(p.y));byte(b,p.facing);byte(b,p.frame);byte(b,p.mood);byte(b,p.shiny);word(b,p.emoteSequence);byte(b,p.emote);}
}
game::CampState campState(Reader& r){
    ...
    while(n--){game::CampMon p;p.species=uint16_t(r.u16());p.x=int16_t(r.u16());p.y=int16_t(r.u16());p.facing=uint8_t(r.u8());p.frame=uint8_t(r.u8());p.mood=uint8_t(r.u8());p.shiny=r.boolean();p.emoteSequence=uint16_t(r.u16());p.emote=uint8_t(r.u8());c.party.push_back(p);}if(!game::validCamp(c))throw std::runtime_error("Invalid camp state");return c;
}
