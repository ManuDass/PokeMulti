...
- **Upstream:** https://github.com/mgba-emu/mgba (mGBA itself is not vendored in this FireRed source-only distribution)
...
subject to the MPL-2.0; the actual modified covered files `src/runtime/bios_hle.cpp` and
`src/runtime/bios_hle.h` are supplied in the installed `license-source/bios_hle`
directory, together with the MPL-2.0 text.

## FireRed integration changes (2026-09-08)

This fork defaults to BIOSless HLE and fails unsupported services explicitly.
It does not ship or compile a real BIOS image. See docs/THIRD_PARTY.md for the
complete local modification list. JRickey MIT terms are reproduced in
UPSTREAM-JRICKEY-MIT.txt.
