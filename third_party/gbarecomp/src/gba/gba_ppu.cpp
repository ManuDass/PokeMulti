...
// world â€” used for non-overworld screens (menus, battles, transitions) so they
...
void GbaPpu::deserialize(gbarecomp::debug::SnapshotReader& r) {
    ...
    vcount_          = r.u16();
    frame_count_     = r.u64();
    if (composition_work_) composition_work_->valid.fill(false);
    if (composition_latched_) composition_latched_->valid.fill(false);
...
void GbaPpu::set_view_margins(uint32_t left, uint32_t right,
    ...
    // top/bottom are forced to 0 (vertical expansion deferred) â€” the params
...
//
// References: GBATEK Â§ "GBA OBJs - OAM", Â§ "GBA Palettes",
//             Â§ "GBA VRAM Character Data".

namespace {

// Sprite shape Ã— size â†’ (width, height) in pixels.
...
inline void to_rgb888(uint16_t c, uint8_t* out) {
    // 5 â†’ 8 bit expansion: ((v << 3) | (v >> 2)) gives full-range 0..255.
    uint8_t r = (c >>  0) & 0x1F;
    uint8_t g = (c >>  5) & 0x1F;
    uint8_t b = (c >> 10) & 0x1F;
...
void render_scanline_internal(uint8_t* rgb,
                              ...
                              uint32_t kScreenWidth,
                              uint32_t kScreenHeight,
                              ObjCompositionFrame* composition = nullptr,
                              ObjOrderProvider order_provider = nullptr) {
    if (y >= kScreenHeight) return;

    if (composition) {
        composition->valid[y] = true;
        std::fill_n(composition->pixels.begin() + y * kScreenWidth, kScreenWidth, CompositionPixel{});
        composition->lines[y] = {load_u16_le(io + 0x50), load_u16_le(io + 0x52), load_u16_le(io + 0x54)};
    }
    ...
    // â”€â”€ Windows (GBATEK "LCD I/O Windows") â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // each line is what carves the circular IRIS used by room transitions â€” it
    ...
    // y within WINnV [Y1,Y2)? GBATEK: Y2>height or Y1>Y2 â†’ Y2=height.
    ...
    // x within WINnH [X1,X2)? GBATEK: X2>width or X1>X2 â†’ X2=width.
    ...
    // â”€â”€ Bitmap BG modes 3/4/5 (GBATEK "LCD VRAM Bitmap BG Modes") â”€â”€â”€â”€â”€â”€â”€
    ...
    // the bitmap stay transparent and show the backdrop â€” that is what
    ...
    if (dispcnt & 0x1000u) {
        ...
        for (int idx = 127; idx >= 0; --idx) {
            ...
            uint32_t composition_order=uint32_t(idx);
            bool have_composition_order=false;
            ...
            auto emit_obj = [&](int tex_x, int tex_y, int screen_x) {
                ...
                bool t1 = blend_enabled(ux) &&
                    obj_mode == 1;
                if (!have_composition_order && composition && order_provider) {
                    composition_order=order_provider(unsigned(idx),entry);
                    have_composition_order=true;
                }
                submit(ux, color, key, 4, t1, obj_target2, composition_order);
            };
            if (rot_scale) {
                int bw = disable_or_double ? sw * 2 : sw;
                int bh = disable_or_double ? sh * 2 : sh;
    ...
    for (uint32_t x = 0; x < kScreenWidth; ++x) {
        uint8_t* dst = row + x * 3;
        if (composition) {
            auto snapshot = [&](const PixelCandidate& p) {
                return CompositionSample{native_composition_key(p.key,p.layer,p.order),
                    p.color,p.layer,p.target1,p.target2,p.valid, p.layer==4?uint16_t(oam[(p.key%256)*8+4]|(oam[(p.key%256)*8+5]<<8)):uint16_t(0xffff)};
            };
            auto& pixel = composition->pixels[y * kScreenWidth + x];
            pixel.top = snapshot(top[x]); pixel.second = snapshot(second[x]);
            pixel.objects = (dispcnt & 0x1000u) && layer_enabled(x,4);
            pixel.effects = blend_enabled(x);
        }
        ...
        // EVA=0/EVB=16 is a valid blend (1st target fully fades into the 2nd) â€”
...
// â”€â”€ Wide (view-expanded) scanline compositor â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// the surrounding map. WIN0/WIN1 register tests are NOT widened â€” a margin hx
...
void render_scanline_wide(uint8_t* rgb, uint32_t y, uint16_t dispcnt,
    ...
    // Bitmap BG modes 3/4/5 â€” see the commentary on render_bitmap_bg in
...
void GbaPpu::render(uint8_t* rgb,
    ...
    if (!view_expanded()) {
        // Faithful path â€” literally unchanged from before view expansion existed.
    ...
    // OAM indices appear on top â€” close enough for the BIOS intro.
    for (int idx = 127; idx >= 0; --idx) {
        const uint8_t* entry = oam + idx * 8;
        uint16_t attr0 = load_u16_le(&entry[0]);
        uint16_t attr1 = load_u16_le(&entry[2]);
        ...
        // OBJ Mode (attr0 bits 10-11) per GBATEK Â§ "GBA OBJs â€” OAM
        // Attribute 0":
        ...
        uint32_t obj_mode = (attr0 >> 10) & 0x3u;
        if (obj_mode == 2) continue;            // window stencil â€” invisible
...
void GbaPpu::render_scanline(uint32_t y,
    ...
    if (!view_expanded()) {
        render_scanline_internal(work_fb_.data(), y, dispcnt,
                                 ...
                                 kScreenWidth, kScreenHeight, composition_work_.get(), obj_order_provider_);
        return;
    }
    if (composition_work_) composition_work_->valid[y] = false;
...
void GbaPpu::latch_framebuffer(uint16_t dispcnt,
                               ...
                               const uint8_t* oam,
                               const uint8_t* pal) {
    if (composition_work_ && !view_expanded()) {
        for (uint32_t y=0; y<kScreenHeight; ++y)
            render_scanline_internal(work_fb_.data(),y,dispcnt,io,vram,oam,pal,
                kScreenWidth,kScreenHeight,composition_work_.get(),obj_order_provider_);
    } else render(work_fb_.data(), dispcnt, io, vram, oam, pal);
    mark_framebuffer_latched();
}

void GbaPpu::mark_framebuffer_latched() {
    if (composition_work_) *composition_latched_ = *composition_work_;
    std::memcpy(latched_fb_.data(), work_fb_.data(), render_bytes());
    has_latched_fb_ = true;
}

void GbaPpu::enable_obj_composition(ObjOrderProvider provider) {
    obj_order_provider_ = provider;
    if (!composition_work_) {
        composition_work_ = std::make_unique<ObjCompositionFrame>();
        composition_latched_ = std::make_unique<ObjCompositionFrame>();
    }
}
void GbaPpu::compose_host_objects(uint8_t* rgb, const HostObjPixel* objects,
                                 uint32_t width, uint32_t height) const {
    if (!rgb || !objects || !composition_latched_ || view_expanded() || width!=240 || height!=160) return;
    for (unsigned y=0; y<160; ++y) {
        if (!composition_latched_->valid[y]) continue;
        const auto& line = composition_latched_->lines[y];
        const unsigned effect = (line.control >> 6) & 3;
        for (unsigned x=0; x<240; ++x) {
            const unsigned i=y*240+x;
            const auto& obj=objects[i]; const auto& source=composition_latched_->pixels[i];
            if (obj.key==kNoHostObject || !source.objects) continue;
            auto top=source.top, second=source.second;
            CompositionSample added{obj.key,obj.color,4,
                source.effects && effect>=2 && (line.control & 0x10)!=0,
                (line.control & 0x1000)!=0,true};
            if (added.key < top.key) { second=top; top=added; }
            else if (added.key < second.key) second=added;
            else continue;
            // UI assets keep their supplied 8-bit channels whenever no native
            // lighting effect transforms the winning object. Visibility still
            // uses the same native priority/window/blanking checks above.
            if (top.key==obj.key && (obj.rgba>>24) && !top.first) {
                const unsigned alpha=obj.rgba>>24;
                for(unsigned channel=0;channel<3;++channel){
                    const unsigned value=(obj.rgba>>(channel*8))&255;
                    rgb[i*3+channel]=uint8_t((value*alpha+unsigned(rgb[i*3+channel])*(255-alpha)+127)/255);
                }
                continue;
            }
            uint16_t color=top.color;
            if ((effect==1 || top.layer==4) && top.first && second.valid && second.second &&
                !(top.layer==4 && second.layer==4))
                color=blend_alpha_gba555(color,second.color,line.alpha&31,(line.alpha>>8)&31);
            else if ((effect==2 || effect==3) && top.first) {
                const unsigned amount=std::min(16u,unsigned(line.brightness&31));
                color=effect==2 ? brighten_gba555(color,amount) : darken_gba555(color,amount);
            }
            to_rgb888(color,rgb+i*3);
        }
    }
}
