...
class GbaIrq;
class GbaAudio;
// Host-provided virtual cable. Only SIO register words cross this interface.
class SerialLink {
public:
    virtual ~SerialLink() = default;
    virtual bool connected() const = 0;
    virtual uint16_t control(uint16_t value) = 0;
    virtual bool start(uint16_t word,std::array<uint16_t,4>& receive) = 0;
    virtual bool poll(uint16_t word,std::array<uint16_t,4>& receive) = 0;
    // A network slave rendezvous at a guest send-word write prevents its guest
    // clock outrunning the master while wall-clock network/compilation waits.
    virtual bool clocked() const { return false; }
    virtual bool clockReady() const { return false; }
    virtual bool request(uint32_t&) { return false; }
    virtual bool tryRequest(uint32_t&) { return false; }
    virtual bool respond(uint16_t,std::array<uint16_t,4>&) { return false; }
    virtual bool startTimed(uint16_t word,uint32_t,std::array<uint16_t,4>& receive) { return start(word,receive); }
    virtual bool synchronizeWrites() const { return false; }
    virtual bool awaitWord(uint16_t word,std::array<uint16_t,4>& receive) { return poll(word,receive); }
};
...
public:
    ...
    void set_serial_link(SerialLink* link) { serial_link_=link; }
...
private:
    ...
    bool sio_link_result_ready_ = false;
    std::array<uint16_t,4> sio_link_words_{};
    GbaPpu*       ppu_   = nullptr;
    GbaIrq*       irq_   = nullptr;
    ...
    uint64_t sio_clock_=0,sio_last_start_=0,sio_target_start_=0;
    bool sio_clock_valid_=false,sio_request_pending_=false;
