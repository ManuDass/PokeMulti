...
public:
    ...
    // Opt-in, native-width host sprite composition against the latched frame.
    void enable_obj_composition(ObjOrderProvider provider);
    void compose_host_objects(uint8_t* rgb, const HostObjPixel* objects,
                              uint32_t width, uint32_t height) const;
    const ObjCompositionFrame* obj_composition_frame() const { return composition_latched_.get(); }

private:
    std::unique_ptr<ObjCompositionFrame> composition_work_, composition_latched_;
    ObjOrderProvider obj_order_provider_ = nullptr;
