...
void GbaIo::tick_sio(uint32_t cycles) {
    sio_clock_+=cycles;
    if(serial_link_ && serial_link_->clocked()) {
        const uint16_t cnt=load_u16(&io_[IoReg::SIOCNT]);
        const bool slave=(cnt&0x3000u)==0x2000u && (cnt&0x30u)!=0;
        const bool synchronized=(cnt&0x4000u)!=0 && serial_link_->clockReady();
        if(slave && !sio_transfer_active_) {
            if(!sio_request_pending_){
                uint32_t elapsed=0;
                // IRQ enable controls notification, not whether the cable shifts
                // data. Initialization and save/return paths also poll transfers.
                const bool pending=synchronized?serial_link_->request(elapsed):serial_link_->tryRequest(elapsed);
                if(pending){
                    sio_target_start_=sio_clock_valid_&&elapsed?sio_last_start_+elapsed:sio_clock_;
                    sio_request_pending_=true;
                }
            }
            if(sio_request_pending_ && sio_clock_>=sio_target_start_){
                sio_link_result_ready_=serial_link_->respond(load_u16(&io_[0x12Au]),sio_link_words_);
                sio_request_pending_=false;sio_last_start_=sio_clock_;sio_clock_valid_=true;
                if(sio_link_result_ready_){
                    static constexpr uint32_t durations[]{63427u,16241u,10998u,5755u};
                    sio_cycles_remaining_=durations[cnt&3u];sio_transfer_active_=true;
                    store_u16(&io_[IoReg::SIOCNT],(cnt&~0x40u)|0x80u);
                    // The interval passed to this tick ended at transfer start.
                    // Charge the transfer duration only on subsequent ticks.
                    return;
                }
            }
        }
    }
    if (!sio_transfer_active_ && serial_link_ && !serial_link_->clocked() && !serial_link_->synchronizeWrites() &&
        (load_u16(&io_[IoReg::SIOCNT]) & 0x3000u)==0x2000u &&
        serial_link_->poll(load_u16(&io_[0x12Au]),sio_link_words_)) {
        sio_link_result_ready_=true;sio_transfer_active_=true;
        static constexpr uint32_t durations[]{63427u,16241u,10998u,5755u};
        sio_cycles_remaining_=durations[load_u16(&io_[IoReg::SIOCNT])&3u];
        store_u16(&io_[IoReg::SIOCNT],load_u16(&io_[IoReg::SIOCNT])|0x80u);
    }
    ...
    if(sio_link_result_ready_) {
        for(unsigned i=0;i<4;++i)store_u16(&io_[IoReg::SIODATA32+i*2],sio_link_words_[i]);
        sio_link_result_ready_=false;
    } else store_u32(&io_[IoReg::SIODATA32], 0xFFFFFFFFu);
...
uint32_t GbaIo::cycles_until_next_sio_event() const {
    if (!sio_transfer_active_) {
        if(sio_request_pending_)return sio_target_start_>sio_clock_?uint32_t(sio_target_start_-sio_clock_):1u;
        return 0xFFFFFFFFu;
    }
...
uint8_t GbaIo::read8(uint32_t off) {
    if(off==IoReg::SIOCNT || off==IoReg::SIOCNT+1) return uint8_t(read16(IoReg::SIOCNT)>>((off&1)*8));
...
uint16_t GbaIo::read16(uint32_t off) {
    if(off==IoReg::SIOCNT && serial_link_) return serial_link_->control(load_u16(&io_[off]));
...
void GbaIo::write8(uint32_t off, uint8_t v) {
    if(off==IoReg::SIOCNT || off==IoReg::SIOCNT+1) {
        uint16_t value=load_u16(&io_[IoReg::SIOCNT]);
        value=(off&1)?uint16_t((value&255)|(uint16_t(v)<<8)):uint16_t((value&0xff00)|v);
        write16(IoReg::SIOCNT,value);return;
    }
...
void GbaIo::write16(uint32_t off, uint16_t v) {
    ...
    switch (off) {
        ...
        case 0x12A: {
            store_u16(&io_[off],v);
            const uint16_t cnt=load_u16(&io_[IoReg::SIOCNT]);
            if(serial_link_ && serial_link_->synchronizeWrites() && serial_link_->connected() &&
               (cnt&0x7000u)==0x6000u && (serial_link_->control(cnt)&0x30u) && !sio_transfer_active_) {
                sio_link_result_ready_=serial_link_->awaitWord(v,sio_link_words_);
                if(!sio_link_result_ready_){store_u16(&io_[IoReg::SIOCNT],cnt|0x40u);return;}
                static constexpr uint32_t durations[]{63427u,16241u,10998u,5755u};
                sio_cycles_remaining_=durations[cnt&3u];sio_transfer_active_=true;
                store_u16(&io_[IoReg::SIOCNT],(cnt&~0x40u)|0x80u);
            }
        ...
        case IoReg::SIOCNT: {
            const uint16_t before=load_u16(&io_[off]);
            if((before&0x7000u)!=(v&0x7000u)){
                sio_clock_valid_=false;sio_request_pending_=false;
                if((v&0x7000u)!=0x6000u){sio_transfer_active_=false;sio_link_result_ready_=false;sio_cycles_remaining_=0;}
            }
            if(serial_link_)v=serial_link_->control(v);
            if(serial_link_ && serial_link_->connected() && (v&0x3000u)==0x2000u) {
                const uint16_t old=load_u16(&io_[off]);
                store_u16(&io_[off],v);
                if((v&0x80u) && !(old&0x80u) && !(v&0x30u)) {
                    const uint32_t elapsed=sio_clock_valid_?uint32_t(sio_clock_-sio_last_start_):0;
                    sio_last_start_=sio_clock_;sio_clock_valid_=true;
                    sio_link_result_ready_=serial_link_->startTimed(load_u16(&io_[0x12Au]),elapsed,sio_link_words_);
                    store_u16(&io_[off],sio_link_result_ready_?uint16_t(v&~0x40u):uint16_t(v|0x40u));
                    static constexpr uint32_t durations[]{63427u,16241u,10998u,5755u};
                    sio_cycles_remaining_=durations[v&3u];sio_transfer_active_=true;
                }
                return;
            }
