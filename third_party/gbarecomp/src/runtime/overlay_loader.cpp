...
void overlay_loader_init(const std::string& cache_root,
    ...
    // Load/validate cached shards on demand through the existing worker. Loading
    // thousands of unrelated DLLs up front made later launches appear frozen.
    const char* preload=std::getenv("GBARECOMP_PRELOAD_NATIVE_CACHE");
    int warm = preload&&preload[0]=='1'?warm_load_cache():0;
    s_stop.store(false);
