...
void find_config_arg(int argc, char** argv, Args* args) {
    for (int i = 1; i < argc; ++i) {
        ...
        if ((s == "--bios" || s == "--rom" || s == "--bios-sha1" ||
             ...
             s == "--dump-png" || s == "--save-state" || s == "--load-state" ||
...
bool parse_cli(int argc, char** argv, Args* args, std::string* err) {
    for (int i = 1; i < argc; ++i) {
        ...
        if (s == "--save-state") {
            const char* v=need_value("--save-state"); if(!v)return false;
            args->save_state=v; continue;
        }
        if (s == "--load-state") {
...
int run_game(int argc, char** argv, const RunOptions& opts) {
    ...
    gba::GbaBios bios;
    if (!opts.biosless_hle && !bios.load_from_file(args.bios, std::string{}, &err)) {
        std::fprintf(stderr, "[gbarecomp:runtime] %s\n", err.c_str());
        return 1;
    }

    std::vector<uint8_t> rom;
    if (opts.rom_data) rom.assign(opts.rom_data, opts.rom_data + opts.rom_size);
    else if (!read_file(args.rom, &rom, &err)) {
        std::fprintf(stderr, "[gbarecomp:runtime] %s\n", err.c_str());
        return 1;
    }
    ...
    if (!args.quiet) {
        if (opts.biosless_hle) std::printf("bios_backend=high-level services (no BIOS image)\n");
        else std::printf("bios_loaded sha1=%s size=%zu\n",
    ...
    gba::GbaBus bus;
    gba::GbaPpu ppu;
    if (!opts.biosless_hle) bus.set_bios(&bios);
    g_runtime_biosless = opts.biosless_hle;
    if (opts.biosless_hle) { args.bios_hle = true; args.bios_skip_intro = 1; }
    ...
    if(opts.biosless_hle){args.bios_hle=true;args.bios_hle_keep_intro=false;args.bios_skip_intro=1;}
    ...
    if (!args.quiet)
        std::printf("bios_backend=%s\n",
                    opts.biosless_hle ? "HLE (no BIOS image; unsupported services stop)" : gba::bios_hle_mode_name(gba::bios_hle_mode()));
    ...
    if (header.save_type == gba::SaveType::SRAM) {
        ...
        std::error_code ec;
        if (!args.save_path.empty() &&
            std::filesystem::exists(std::filesystem::u8path(args.save_path), ec)) {
...
#ifdef _WIN32
        const auto target = std::filesystem::u8path(args.save_path);
        const auto temporary = std::filesystem::u8path(tmp);
        if (std::filesystem::exists(target) &&
            !CopyFileW(target.c_str(), std::filesystem::u8path(args.save_path + ".bak").c_str(), FALSE)) {
            std::fprintf(stderr,"[gbarecomp:runtime] save backup failed (%lu); preserving original\n",GetLastError());
            return false;
        }
        if (!MoveFileExW(temporary.c_str(),target.c_str(),MOVEFILE_REPLACE_EXISTING | MOVEFILE_WRITE_THROUGH)) {
            std::fprintf(stderr,"[gbarecomp:runtime] atomic save replacement failed (%lu); preserving original\n",GetLastError());
            return false;
        }
#else
        std::error_code ec;
        std::filesystem::rename(tmp,args.save_path,ec);
        if (ec) return false;
#endif
    ...
    runtime_init(&bus);
    reset_recomp_cpu();
    if (opts.game_ready) opts.game_ready();
    ...
    auto apply_input_replay = [&]() {
        ...
        if (input_replay_events[input_replay_index].frame <= frame)
            bus.io().set_keyinput(
                opts.game_input?opts.game_input(input_replay_events[input_replay_index].keyinput):input_replay_events[input_replay_index].keyinput);
    };
    ...
    auto pump_host_input = [&]() {
        ...
        if (!input_replay_requested) bus.io().set_keyinput(opts.game_input?opts.game_input(ev.keyinput):ev.keyinput);
        if (bus.gyro().active()) {
    ...
    if (opts.extended_view_frame || opts.game_frame) {
        runtime_set_frame_start_hook([&]() {
            if (opts.extended_view_frame) publish_extended_view_frame();
            if (opts.game_frame) opts.game_frame(ppu.frame_count());
        });
        if(opts.extended_view_frame) publish_extended_view_frame();
    }
    ...
    if (args.window && present_in_place) {
        ...
        runtime_set_frame_present_hook([&]() -> bool {
            ...
            if (frame != last_presented_frame) {
                ...
                if (opts.game_render) opts.game_render(live_fb.data(),ppu.render_width(),160u);
    ...
    if (!args.save_state.empty()) {
        std::string error;
        if(!do_savestate_save(args.save_state,error)) {std::fprintf(stderr,"State capture failed: %s\n",error.c_str());save_ok=false;}
    }
    ...
    if (!args.dump_bmp.empty() || !args.dump_png.empty()) {
        ...
        if(opts.game_render) opts.game_render(fb.data(),ppu.render_width(),160u);
