...
struct RunOptions {
    HostShell shell{};
    // Local integration: explicit open-source BIOS service backend, no BIOS image.
    bool biosless_hle = false;
    void (*game_ready)() = nullptr;
    uint16_t (*game_input)(uint16_t) = nullptr;
    void (*game_frame)(std::uint64_t) = nullptr;
    void (*game_render)(std::uint8_t*,std::uint32_t,std::uint32_t) = nullptr;
