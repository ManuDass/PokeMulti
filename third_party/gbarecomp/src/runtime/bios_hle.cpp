// FireRed fork modification notice (2026-09-08): BIOSless boot, IRQ wait and
// service integration added; actual modified source accompanies binaries.
// Upstream mGBA attribution and MPL-2.0 coverage are retained.
...
// source attribution in THIRD_PARTY_ATTRIBUTION.md) and GBATEK. See THIRD_PARTY_ATTRIBUTION.md.
//
...
inline int clz32(uint32_t x) { return std::countl_zero(x); }
...
int dispatch(uint32_t swi) {
    ...
    switch (swi) {
    case SWI_SOFT_RESET: {
        if (!g_runtime_biosless) return 0;
        const bool ram = rd8(0x03007FFAu) != 0;
        for (uint32_t a = 0x03007E00u; a < 0x03008000u; a += 4u) wr32(a, 0);
        bios_hle_boot_skip(ram ? 0x02000000u : 0x08000000u);
        break;
    }
    case SWI_REGISTER_RAM_RESET: {
        if (!g_runtime_biosless) return 0;
        const uint32_t flags = R(0);
        wr16(0x04000000u, 0x80u);
        const uint32_t starts[] = {0x02000000u,0x03000000u,0x05000000u,0x06000000u,0x07000000u};
        const uint32_t sizes[] = {0x40000u,0x7E00u,0x400u,0x18000u,0x400u};
        for (unsigned i=0;i<5;++i) if (flags & (1u<<i))
            for (uint32_t off=0;off<sizes[i];off+=4u) wr32(starts[i]+off,0);
        if (flags & 0x20u) for (uint32_t a=0x04000120u;a<=0x0400012Au;a+=2u) wr16(a,0);
        if (flags & 0x40u) {
            wr16(0x04000084u,0x80u);
            for (uint32_t a=0x04000060u;a<0x04000084u;a+=2u) wr16(a,0);
            wr16(0x04000082u,0x8800u); wr16(0x04000084u,0); wr16(0x04000088u,0x200u);
        }
        if (flags & 0x80u) {
            for (uint32_t a=0x04000008u;a<0x04000060u;a+=2u) wr16(a,0);
            wr16(0x04000020u,0x100u); wr16(0x04000026u,0x100u);
            wr16(0x04000030u,0x100u); wr16(0x04000036u,0x100u);
            for (uint32_t a=0x040000B0u;a<0x04000110u;a+=2u) wr16(a,0);
            wr16(0x04000200u,0); wr16(0x04000202u,0xFFFFu); wr16(0x04000208u,0);
        }
        break;
    }
    case SWI_HALT: case SWI_STOP:
        if (!g_runtime_biosless) return 0;
        wr8(0x04000301u, swi == SWI_STOP ? 0x80u : 0);
        break;
    case SWI_INTR_WAIT: case SWI_VBLANK_INTR_WAIT: {
        if (!g_runtime_biosless) return 0;
        const uint16_t mask = swi == SWI_VBLANK_INTR_WAIT ? 1u : static_cast<uint16_t>(R(1));
        const bool discard = swi == SWI_VBLANK_INTR_WAIT || R(0) != 0;
        if (discard) wr16(0x03007FF8u, rd16(0x03007FF8u) & ~mask);
        wr16(0x04000208u,1);
        uint32_t budget = 0;
        while (!(rd16(0x03007FF8u) & mask)) {
            runtime_tick(128);
            if (++budget > 5u * 16777216u / 128u)
                throw std::runtime_error("BIOS IntrWait stalled: requested IRQ was not acknowledged by the game");
        }
        wr16(0x03007FF8u, rd16(0x03007FF8u) & ~mask);
        break;
    }
    ...
    // Real BIOS services remain interruptible. A bulk device tick delayed
    // serial/timer IRQ delivery until decompression ended and missed whole
    // FireRed link frames during map loads. Preserve the cost while allowing
    // pending IRQs to run throughout the service's charged time.
    while(cost){const auto slice=std::min(cost,128u);runtime_tick(slice);cost-=slice;}
    return 1;
}
