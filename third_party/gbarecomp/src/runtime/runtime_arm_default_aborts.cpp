...
void self_heal_write_report(const char* frag_path) {
    ...
    if (g_misses.empty() && healed == 0) {
        std::printf("self_heal_coverage=NO_DISPATCH_MISSES dispatch_misses=0 "
...
std::string self_heal_misses_json() {
    ...
    std::snprintf(buf, sizeof(buf),
        ...
        fully_static ? "NO_DISPATCH_MISSES" : "NOT_STATIC",
...
extern "C" int runtime_bridge_interpret(uint32_t entry_pc, bool entry_thumb,
    ...
    for (;;) {
        ...
        // Local BIOSless integration: long-running top-level cartridge code
        // must honor frame/HALT yields even when no static corpus is present.
        if (g_runtime_biosless && !irq_cont && max_instrs == 0 && runtime_should_yield()) break;
