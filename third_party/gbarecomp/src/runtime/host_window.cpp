...
void HostWindow::push_audio_samples(const int16_t* samples, std::size_t count) {
    ...
    if (b->shell.volume) b->volume = std::clamp(b->shell.volume(), 0, 100);
...
void HostWindow::set_shell(HostShell shell) {
    if (!open_ || !impl_) return;
    auto* b = static_cast<Backend*>(impl_);
    b->shell = shell;
    if (shell.open) { SDL_RenderSetLogicalSize(b->renderer, 0, 0); shell.open(b->window, b->renderer); }
}

void HostWindow::close() {
    if (!impl_) { open_ = false; return; }
    auto* b = static_cast<Backend*>(impl_);
    if (b->shell.close) { b->shell.close(); b->shell = {}; }
...
void HostWindow::present(const uint8_t* rgb888) {
    ...
    if (b->shell.render) b->shell.render(b->texture);
    else {
#if defined(__ANDROID__)
    int drawable_w = 0;
    int drawable_h = 0;
...
#endif
    } // product shell owns the entire frame when attached
...
HostWindow::Events HostWindow::pump() {
    ...
    while (SDL_PollEvent(&e)) {
        if (b->shell.event) b->shell.event(&e);
        if (b->shell.captures_input && b->shell.captures_input() &&
            (e.type == SDL_KEYDOWN || e.type == SDL_KEYUP || e.type == SDL_TEXTINPUT)) continue;
#if defined(GBARECOMP_RUNTIME_UI)
...
#endif
        if (e.type == SDL_QUIT) {
            ev.quit = !b->shell.allow_quit || b->shell.allow_quit();
    ...
    if (b->shell.wasd_movement) {
        const SDL_Scancode aliases[]{SDL_SCANCODE_D, SDL_SCANCODE_A, SDL_SCANCODE_W, SDL_SCANCODE_S};
        for (int i=0;i<4;++i) if (ks[aliases[i]]) keys &= static_cast<uint16_t>(~(1u << (4+i)));
    }
    ...
    if (b->shell.controller_keys) keys &= b->shell.controller_keys();
    ev.keyinput = keys;
    if (b->shell.captures_input && b->shell.captures_input()) ev.keyinput = 0x03FFu;
#if defined(GBARECOMP_RUNTIME_UI)
    if (b->runtime_ui && recomp_runtime_ui_is_open(b->runtime_ui))
        ev.keyinput = 0x03FFu;
#endif
...
void HostWindow::set_shell(HostShell) {}
