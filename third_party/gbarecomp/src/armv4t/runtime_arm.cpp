...
extern "C" bool g_runtime_biosless = false;
...
extern "C" void runtime_irq(uint32_t return_address) {
    ...
    // Original semantic IRQ trampoline, using the documented guest IRQ ABI.
    // 0xFFFF0000 is an internal return marker, never fetched as guest code.
    if (g_runtime_biosless) {
        const uint32_t sp = g_cpu.R[13] - 24u;
        for (unsigned i = 0; i < 4; ++i) bus_write_u32(sp + i * 4u, g_cpu.R[i]);
        bus_write_u32(sp + 16u, g_cpu.R[12]);
        bus_write_u32(sp + 20u, g_cpu.R[14]);
        g_cpu.R[13] = sp;
        g_cpu.R[0] = 0x04000000u;
        g_cpu.R[14] = 0xFFFF0000u;
    ...
    auto drive_irq_instruction = []() {
        ...
        if (g_runtime_biosless && pc == 0xFFFF0000u) {
            const uint32_t sp = g_cpu.R[13];
            for (unsigned i = 0; i < 4; ++i) g_cpu.R[i] = bus_read_u32(sp + i * 4u);
            g_cpu.R[12] = bus_read_u32(sp + 16u);
            const uint32_t lr = bus_read_u32(sp + 20u);
            g_cpu.R[14] = lr;
            g_cpu.R[13] += 24u;
            runtime_exception_return(lr - 4u);
            return;
        }
        const int thumb = (g_cpu.cpsr & CPSR_T_BIT) != 0;
        if (g_runtime_biosless || g_force_interp ||
