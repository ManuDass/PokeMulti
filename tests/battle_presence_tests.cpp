...
int main(){try{
    ...
    auto pose=battleMonPose(r.battle,r.battle.mons[0],1);check(pose.follower==1&&pose.followerFacing==4,"Combat pose carries actual species and facing");r.battle.mons[0].visible=false;check(!battleMonPose(r.battle,r.battle.mons[0],1).active,"Fainted Pokemon stops appearing");r.battle.mons[0].species=4;r.battle.mons[0].visible=true;check(battleMonPose(r.battle,r.battle.mons[0],1).follower==4,"Replacement becomes the battle actor");
    ...
    r.battle.settled=false;r.battle.mons[0].walking=true;r.battle.mons[0].frame=2;++r.sequence;guest.updateWorld(r);until([&]{return late.status().world.battles[1].mons[0].walking;});check(late.status().world.battles[1].mons[0].frame==2&&!late.status().world.battles[1].settled,"Walking phase/frame lost on wire");
    r.battle.mons[0].visible=false;++r.sequence;guest.updateWorld(r);until([&]{return !host.status().world.battles[1].mons[0].visible;});r.battle.mons[0].species=7;r.battle.mons[0].visible=true;++r.sequence;guest.updateWorld(r);until([&]{return late.status().world.battles[1].mons[0].species==7&&late.status().world.battles[1].mons[0].visible;});
