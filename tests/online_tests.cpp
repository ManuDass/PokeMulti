...
int main(){
    try{
        ...
        fr::game::PlayerState p;p.active=true;p.x=17;p.y=12;p.mapGroup=3;p.mapNumber=1;p.elevation=3;p.follower=25;p.pixelX=273;p.pixelY=193;p.spriteFrame=7;p.flip=1;p.sequence=42;p.sampleTime=234567;p.followerFacing=3;p.followerFrame=2;p.partyCount=6;p.partyEggs=0x22;p.followerShiny=true;p.followerToken=123456;p.followerEmote=5;p.followerEmoteSequence=5;
        two.update(p);const int twoSlot=two.status().slot;
        until([&]{for(const auto& peer:one.status().peers)if(peer.slot==twoSlot&&peer.player.x==17&&peer.player.follower==25&&peer.player.pixelX==273&&peer.player.spriteFrame==7&&peer.player.flip==1&&peer.player.sequence==42&&peer.player.sampleTime==234567&&peer.player.followerFacing==3&&peer.player.followerFrame==2&&peer.player.partyCount==6&&peer.player.partyEggs==0x22&&peer.player.followerShiny&&peer.player.followerToken==123456&&peer.player.followerEmote==5&&peer.player.followerEmoteSequence==5)return true;return false;});
