...
int main(){try{
    ...
    ReleaseBook manual;manual.manualSaving(true);manual.open(dir/"manual.cfg");check(manual.offer(m)&&!std::filesystem::exists(dir/"manual.cfg"),"Manual release must not save automatically");manual.save();ReleaseBook manualResume;manualResume.open(dir/"manual.cfg");check(manualResume.find(m.id)!=nullptr,"Manual Save persists released population");
