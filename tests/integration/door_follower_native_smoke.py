...
try:
 ...
 key('a',0x37f,20,45);capture('walking-from-door')
 tail=[list(map(int,l.split(','))) for l in read('a','follower-jumps.csv').splitlines() if l.count(',')==8][-30:]
 assert tail and tail[-1][2]>rows[-1][2] and all(p[5]==door_x and p[6]==p[2]-16 and p[8]==1 for p in tail),'Follower did not resume trailing after exit'
 print('PASS: native doorway anchor, forward-facing cardinal exit, continued following, no snapping and no overlays during fade',flush=True)
