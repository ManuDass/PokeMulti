...
The friend package includes 386 normal and 386 shiny directional follower PNGs under `LocalAssets/Followers` and `LocalAssets/Followers shiny`, plus the four supplied effect sheets under `LocalAssets/Follower effects`. Send-out, recall and emote effects display at half source size with nearest-neighbor sampling; the PNG files are unchanged. The explicit list is `cmake/FollowerAssets.cmake`; every PNG is copied byte for byte from the owner-supplied Following Pokemon EX 2.5.1 folder. The original plugin metadata and credits are included as `licenses/Following-Pokemon-EX-credits.txt`. No plugin scripts, unused alternate packs, Essentials engine files or ROM are included.
