...
Releases and recaptures automatically save the native game. Personal money, party and items remain personal. The shared wild population is stored in the hosting trainer's profile (`released-world.cfg`) and resumes when that profile hosts again. Solo releases have a separate local population (`released-solo.cfg`). A disconnected encounter remains reserved until its trainer reconnects and the local battle/save receipt is resolved.
